-- GTM Supabase Database Fix
-- Исправленная схема без проблемных RLS политик
...
-- Простые RLS политики (без проблемных функций)
...
CREATE POLICY "Service role full access" ON artists FOR ALL USING (true);
CREATE POLICY "Service role full access" ON artist_gallery FOR ALL USING (true);
CREATE POLICY "Service role full access" ON giveaways FOR ALL USING (true);
CREATE POLICY "Service role full access" ON giveaway_tickets FOR ALL USING (true);
...
INSERT INTO artists (name, username, bio, city, is_active) 
VALUES ('Test Artist', 'test_artist', 'Test bio', 'Moscow', true)
